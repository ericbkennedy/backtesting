    ...
    for row in reader:
        if len(row) == 5:
            ...
            if closingDate != "timestamp":  # skip header
                ...
                months.insert(0, monthlyData)  # insert older months at start
        else:
...
trendFollowingOutperformingDate = months[0][0]  # track if/when buy-and-hold outperforms
...
for index, (closingDate, openPrice, closePrice, dividend) in enumerate(months):
    ...
    if dividend > 0.0:  # reinvest dividend if long
        ...
        if shares > 0.0:  # will get the dividend if long but could sell based on MA
        ...
        if closePrice >= movingAverage:
            ...
            if shares > 0.0:
                ...
                if isTaxableAccount and cash > costBasis:
                    taxes = round((cash - costBasis) * CAPITAL_GAIN_RATE, 2)
                    gain = round(cash - costBasis)
                    comments += (f" and paid {round(taxes)} on gain {gain}")
                    cash -= taxes
                    costBasis = cash
                shares = 0
...
print(
    f"{ticker} buy and hold {buyHoldReturn}% vs {trendFollowingReturn}% for trend following since {initialMonth}"
)
